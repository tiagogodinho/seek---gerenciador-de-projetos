module PessoasHelper
  ...
  #
  def link_to_users(pessoa, options={})
    raise "Invalid users" unless pessoa
    options.reverse_merge! :content_method => :Login, :title_method => :Login, :class => :nickname
    content_text      = options.delete(:content_text)
    content_text    ||= pessoa.send(options.delete(:content_method))
    options[:title] ||= pessoa.send(options.delete(:title_method))
    link_to h(content_text), pessoa_path(pessoa), options
  end
  ...
  def link_to_current_users(options={})
    if current_users
      #$link_to options[:content_method], pessoa_path(current_users)
